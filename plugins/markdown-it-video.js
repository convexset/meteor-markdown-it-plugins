...
function video_embed(md) {
    function video_return(state, silent) {
        // alert('[video_return] ' + EJSON.stringify({state: state, silent: silent}))
        // console.log('[video_return]', ({state: state, silent: silent}))
        var code,
            serviceEnd,
            serviceStart,
            pos,
        ...
        if (state.src.charCodeAt(state.pos) !== 0x40/* @ */) {
            // console.log('state.src.charCodeAt(state.pos) !== 0x40 ==> false')
            return false;
        }
        if (state.src.charCodeAt(state.pos + 1) !== 0x5B/* [ */) {
            // console.log('state.src.charCodeAt(state.pos + 1) !== 0x5B ==> false')
            return false;
        }

        // var match = EMBED_REGEX.exec(state.src);
        var balanceSrc = state.src.substr(oldPos);
        var match = EMBED_REGEX.exec(balanceSrc);
        ...
        if (match.length < 3){
            // console.log("match.length < 3; match: " + match)
            return false;
        }
...
function tokenize_video(md) {
    function tokenize_return(tokens, idx, options, env, self) {
        // alert('[tokenize_return] ' + EJSON.stringify({tokens: tokens, idx: idx, options: options, env: env, self: self}))
...
_MarkdownItPlugins['markdown-it-video'] = {
    plugin: video_plugin
};